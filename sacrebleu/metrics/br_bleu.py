...
class BR_BLEUSignature(Signature):
    def __init__(self, args):
        ...
        smooth_def = BR_BLEU.SMOOTH_DEFAULTS[smooth_str]
...
class BR_BLEU:
    ...
    def extract_ngrams(line, min_order=1, max_order=NGRAM_ORDER) -> Counter:
        ...
        line = line.replace('< eol >','<eol>').replace('< eob >','<eob>')
        #print(line)
        tokens = line.split()
        #print(tokens)
        for n in range(min_order, max_order + 1):
            for i in range(0, len(tokens) - n + 1):
                ...
                if any(x in ngram for x in ["<eol>", "<eob>"]):
                    ngrams[ngram] += 1
    ...
    def reference_stats(refs, output_len):
        ...
        for ref in refs:
            ...
            #print("REF")
            ngrams_ref = BR_BLEU.extract_ngrams(ref)
    ...
    def compute_bleu(correct: List[int],
        ...
        """
        assert smooth_method in BR_BLEU.SMOOTH_DEFAULTS.keys(), \
            "Unknown smooth_method '{}'".format(smooth_method)

        # Fetch the default value for floor and add-k
        if smooth_value is None:
            smooth_value = BR_BLEU.SMOOTH_DEFAULTS[smooth_method]

        precisions = [0.0 for x in range(BR_BLEU.NGRAM_ORDER)]

        smooth_mteval = 1.
        effective_order = BR_BLEU.NGRAM_ORDER
        for n in range(1, BR_BLEU.NGRAM_ORDER + 1):
    ...
    def sentence_score(self, hypothesis: str,
                       references: List[str],
                       use_effective_order: bool = True) -> BR_BLEUScore:
        """
    ...
    def corpus_score(self, sys_stream: Union[str, Iterable[str]],
                     ...
                     use_effective_order: bool = False) -> BR_BLEUScore:
        ...
        for output, *refs in zip(sys_stream, *ref_streams):
            ...
            ref_ngrams, closest_diff, closest_len = BR_BLEU.reference_stats(refs, output_len)

            sys_len += output_len
            ref_len += closest_len

            #print("HYP")
            sys_ngrams = BR_BLEU.extract_ngrams(output)
